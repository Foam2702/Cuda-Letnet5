...
//                     int input_idx = calculateIndex(batch_idx, input_channel_idx, input_row, input_col, height, width);
//                     int kernel_idx = calculateIndex(output_feature_idx, input_channel_idx, kernel_row, kernel_col, kernel_size, kernel_size);
...
__global__ void convolutionForwardKernel(float *output, const float *input, const float *kernel,
                                                  ...
                                                  const int height_in, const int width_in, const int kernel_height)
{
    ...
    GpuTimer time_kernel;
    time_kernel.Start();
    convolutionForwardKernel<<<num_blocks_in_grid, num_threads_per_block>>>(device_output, device_input, device_weight, num_samples, output_channel, input_channel, height_in, width_in, kernel_height);
    time_kernel.Stop();
    float time_kernel_ms = time_kernel.Elapsed();
    std::cout << "\t - Kernel Time: " << time_kernel_ms << " ms" << std::endl;
    // Copy the output back to host
